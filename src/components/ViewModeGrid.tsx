...
export function ViewModeGrid() {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    switch (e.key) {
      case 'ArrowRight': e.preventDefault(); next = Math.min(viewImageIndex + 1, imageFiles.length - 1); break;
      case 'ArrowLeft':  e.preventDefault(); next = Math.max(viewImageIndex - 1, 0); break;
      case 'ArrowDown':  e.preventDefault(); next = Math.min(viewImageIndex + cols, imageFiles.length - 1); break;
      case 'ArrowUp':    e.preventDefault(); next = Math.max(viewImageIndex - cols, 0); break;
      case 'Home':       e.preventDefault(); next = 0; break;
      case 'End':        e.preventDefault(); next = imageFiles.length - 1; break;
      case 'Enter':      e.preventDefault(); openInLargeView(viewImageIndex); return;
      default: return;
    }

    setViewImageIndex(next);
  }, [viewImageIndex, imageFiles.length, getColCount, setViewImageIndex, openInLargeView]);
  ...
  return (
    <div className="view-grid" ref={gridRef} tabIndex={0}>
      {imageFiles.map((file, idx) => (
        ...
        >
          <div className="view-grid__thumb">
            {file.media_type === 'video' ? (
              <VideoThumb path={file.path} className="view-grid__thumb-video" />
            ) : (
              <img
          ...
          <div className="view-grid__name" title={file.name}>{file.name}</div>
        </div>
      ))}
    </div>
  );
